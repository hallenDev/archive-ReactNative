...
import { checkIsIncludeAllNotifType } from './checkIsIncludeNotifType'
...
export const notificationMapper = message => {
  ...
  if (checkIsIncludeAllNotifType(data.msg_type)) {
    ...
    if (data.msg_type === MessageType.NEW_MESSAGE) {
      ...
      data.notification.photoUrl =
        data.payload.users[0].pic || data.payload.users[0].profile_pic
      data.notification.duid = data.payload.users[0].duid
      data.notification.message = data.payload.threads[0].message
      data.notification.msg_system_id =
        data?.payload?.messages[0]?.msg_system_id
    ...
    data.notification.photoUrl =
      data.payload.user.pic || data.payload.user.profile_pic
